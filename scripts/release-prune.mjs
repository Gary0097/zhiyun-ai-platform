...
export function planPrune (names, keepVersions = 2) {
...
export function pruneDist (distDir, { keep = 2, dryRun = false } = {}) {
  const doomed = planPrune(readdirSync(distDir), keep)
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  if (process.argv.includes('--check')) {
    ...
    for (const name of pruneDist(distDir, { keep, dryRun })) {
