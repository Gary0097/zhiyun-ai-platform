(function () {
  const pluginId = "qwenpaw-creator-studio";
  ...
  const CREATOR_BUILD_ID = "1ed2e2d8fa32";
  ...
  function CreatorFrame() {
    ...
    if (!initialSrcRef.current) {
      const appUrl = host.getApiUrl(
        "/frontend_plugin/qwenpaw-creator-studio/files/ui/dist/app/index.html",
      );
  ...
  QwenPaw.registerRoutes(pluginId, [
    {
      path: "/apps/qwenpaw-creator-studio",
      component: CreatorFrame,
      label: "QwenPaw Creator 原版",
      icon: "🎬",
    },
  ]);

  console.info("[qwenpaw-creator-studio] registered route /apps/qwenpaw-creator-studio");
})();
