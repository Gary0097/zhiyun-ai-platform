...
v0.7.0 新增幂等迁移日志、数据库完整性与容量健康、SHA-256 校验备份、可选 AES-GCM 加密备份，以及恢复前强制确认和自动安全备份。加密密钥仅从管理员指定的环境变量读取，不写入 Workspace、日志或仓库。
v0.8.0 新增统一 Excel/CSV 导出能力：支持 .xlsx 与 .csv 两种导出格式，导出前端字段按 Schema Registry 动态生成，且严格遵循 data_mode（demo/production）隔离；内置重复校验，可在导入前发现同批次内唯一键冲突；同时补齐旧版 .xls 解析（需可选安装 xlrd）。
...
- Excel/CSV 导入与导出统一复用同一套解析、字段映射、校验和来源隔离能力。
- 模拟数据按批次撤销，不影响真实数据。
