(function () {
  ...
  function AuditViewer() {
    ...
    var agentSessionRef = React.useRef("audit-" + Date.now().toString(36));
    function agentCommand(key, label) {
      var prompt = key === "chain" ? "帮我查看当前审计链完整性和最近的可疑执行记录"
        : key === "failed" ? "帮我筛选出失败或被阻断的审计记录"
        : key === "integrity" ? "帮我执行审计链完整性校验"
        : (label || key);
    ...
    function startAgentChat(text) {
      ...
      agentAdd("bot", "", null);
      setAgentBusy(true);
      zyPushAgent({ app_id: "zhiyun-audit", kind: "chat", label: text, summary: { records: records, integrity: integrity, status: status }, source_type: "real" });
      ...
      Q.host.fetch("/zhiyun-audit/agent/chat", {
        method: "POST",
        headers: agentHeaders,
        body: JSON.stringify({ text: text, session_id: agentSessionRef.current, user_id: "default", app_id: "zhiyun-audit", history: history })
      })
