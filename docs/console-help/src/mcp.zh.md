...
智造云 AIOS 通过 **MCP（模型上下文协议）** 连接外部服务，并提供一组 **内置工具**，让智能体能够访问文件系统、执行命令、浏览网页等。
...
1. **内置工具**：开箱即用，由 智造云 AIOS 核心提供，如文件操作、命令执行、浏览器自动化等
...
**MCP（模型上下文协议，Model Context Protocol）** 允许 智造云 AIOS 连接到外部 MCP 服务器，扩展智能体访问文件系统、数据库、API 等外部资源的能力。
...
为 智造云 AIOS 原生智能体配置邮箱后，系统会在该智能体工作区自动创建并启用
`qwenpawmail` MCP 驱动卡。驱动卡不保存明文凭据，只引用工作区 `credentials.yaml`
中的加密记录；系统在启动 MCP 子进程时才解析凭据并注入运行环境。
它通过 stdio 启动 `qwenpawmail-mcp` 子包，同时注入状态目录和工作区目录。
该客户端提供 22 个收信、发信、附件、整理、线程和统计工具。

新生成的驱动卡默认访问策略为 `ask`。可以在 **工作区 → MCP** 中按工具和调用来源调整；
重新保存邮箱配置时，已有启用状态、工具范围和访问策略会被保留。

这个客户端不需要通过通用 MCP 页面手动新建，但源码安装必须包含邮箱子包。智能体还应载入
内置 `mailbox` Skill，才能获得账户连接、联系人维护和智能处理功能。完整安装、功能
和安全说明请看 [邮箱管理与自动化](./mailbox)。
...
智造云 AIOS 提供了一组开箱即用的内置工具，智能体可以直接调用这些工具完成各种任务。
...
> 建议只启用实际需要的工具，以减少上下文占用，加快响应速度。配置变更自动热加载，无需重启服务——只有切换 `browser` 工具的新版 / 旧版实现需要重启服务，详见 [浏览器](./browser)。
...
- 智造云 AIOS 会在合适的时候调用 `delegate_external_agent`，与外部智能体建立连续对话，并将中间进度和结果回传到当前会话中
...
- `browser(code)`：用异步 Python 调用 智造云 AIOS 内置 Browser SDK 驱动真实浏览器——导航、定位元素、执行动作、读取可见页面状态。完整 SDK API 会在 agent 首次使用时自动交付。
  - 既可以使用 智造云 AIOS 启动的独立浏览器，也可以接入你自己已登录的 Chrome（需安装 [Chrome 浏览器扩展](./chrome)）
