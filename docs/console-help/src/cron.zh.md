...
在 智造云 AIOS 里，「定时任务（cron job）」用于让系统在指定时间自动执行动作，比如：
...
2. 填写信息：
   ...
   - **任务类型及内容**
     ...
     - 选择**agent**：填写**请求内容**，会定时向智造云 AIOS转发content.text中的请求文本
...
创建定时任务最简单的方式是直接与 智造云 AIOS 对话，让智造云 AIOS帮忙创建：
...
智造云 AIOS 使用五段式 Cron：**分 时 日 月 周**（无秒）。
