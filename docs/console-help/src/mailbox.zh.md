...
智造云 AIOS 可以为每个智能体连接一个独立邮箱，通过 IMAP/SMTP 完成收信、搜索、发信、
...
> 邮箱管理仅支持 智造云 AIOS 原生后端，第三方智能体后端不能配置邮箱。每个智能体的邮箱凭据、
...
3. 新安装会包含 qwenpawmail MCP。源码开发环境可在仓库根目录运行 `make install-dev`；如果
   智造云 AIOS 已安装而邮箱子包缺失，可运行 `make install-mail-mcp`。Docker 镜像会随主项目安装
   邮箱子包。
4. 在 **设置 → 技能池** 中确认内置 `mailbox` Skill 已是最新版本，再到目标智能体的
   **工作区 → 技能** 中载入并启用它。旧版本用户应更新技能池，让 `mailbox` 替换已废弃的
   `himalaya` Skill。

保存邮箱配置后，智造云 AIOS 会自动创建并启用 `qwenpawmail` MCP 驱动卡，同时初始化邮箱工作区
...
智造云 AIOS 当前的托管邮箱流程支持以下 9 个个人邮箱域名：
...
当前 智造云 AIOS 托管流程不支持企业邮箱、自定义域名和 Microsoft 邮箱。

> qwenpawmail MCP 子包也可以脱离 智造云 AIOS 独立使用。独立部署时可用
...
- 智能体会优先在浏览器中打开服务商注册页面。遇到密码、手机号、图片/短信验证码、滑块和
  协议确认时，由你直接在网页中完成必要的人工步骤。这些注册信息不会保存到 智造云 AIOS 配置中。
- 注册完成后，在服务商设置中启用 IMAP/SMTP 并生成授权码，再验证收发信连接。
- 重新打开智能体配置，保持选择 **为智能体配备专属邮箱**，填写最终邮箱名和凭据后保存。
  智造云 AIOS 会自动把 `is_new_account` 设为 `false`、加密保存 secret、同步托管 DriverCard
  并重载智能体。
...
选择 **每封唤醒** 后，智造云 AIOS 监控 `INBOX`。服务商支持时优先使用 IMAP IDLE，服务商
...
配置邮箱后，智造云 AIOS 会在文件缺失时创建这两个工作区文件；已有内容不会被覆盖。
...
- 确认智能体使用 智造云 AIOS 原生后端并已保存邮箱配置；
- 确认安装了包含 `qwenpawmail-mcp` 的当前 智造云 AIOS 版本；
...
部分服务商只实现了 IMAP 的子集，智造云 AIOS 在遇到此类情况时会尝试使用可用条件或本地线程主题降级：
