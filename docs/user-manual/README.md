...
> 适用版本：v1.5.x 及以上（底层运行时 QwenPaw 2.1.0）
...
- 在底座之上预装了 **19 个业务应用（PawApp）**：统一数据中心、智能订单中心、企业数据分析中心、智能财务中心等，覆盖制造企业经营的主要环节。
...
- 所有敏感操作有**审计与灾难性操作阻断**保护；业务功能按账号隔离（会话、任务与知识按登录账号隔离）。
...
| --- | --- | --- |
| 1 | 欢迎页 | 阅读流程说明，点"下一步" |
| 2 | 选择目录 | 默认 `C:\zhizaoyunAIOS`，可更换（建议不含空格的路径）；向导会自动检查剩余空间 ≥ 3 GB |
...
双击 `install-oneclick.cmd`（需已安装 Node.js 20+ 与 Git）。脚本自动完成：安装运行时与锁定应用 → 启动服务 → 等待就绪后打开浏览器 → 创建桌面快捷方式。
...
- 忘记密码请联系系统管理员重置。
- 会话、任务与知识按账号隔离——每个员工登录后看到的是自己的工作台。
...
**Q6：安装/启动日志在哪？**
- 安装向导：安装目录 `install-wizard.log`
- 静默安装：安装目录 `install-log.txt`
- 启动器拉起服务：安装目录 `launcher-service.log`
- 服务运行日志：`apps\zhizaoyunAIOS\workspace\workspaces\<智能体>\logs\`

**Q7：如何升级？**
用新版本安装包装到同一目录，安装程序会自动停旧服务→覆盖→重启，数据保留。升级前建议在 设置 → 备份 做一次工作区备份。

**Q8：如何彻底卸载？**
控制面板 → 应用 → 灵泽万川智造云 AI-OS → 卸载（或运行安装目录 `卸载智造云AI-OS.cmd`），再手动删除安装目录。Workspace 数据在安装目录内，删除目录即全部清除——**卸载前请确认不再需要数据或已备份**。

**Q9：局域网同事能访问吗？**
能。运行 `start-hub.cmd`（0.0.0.0:8000 网关模式）或直接放行 8088 端口；同事浏览器访问 `http://<主机IP>:8088`，各自账号登录、数据按账号隔离。

**Q10：想做演示数据给领导看？**
运行 `python scripts/qa/generate_demo_dataset.py` 注入 2025-12-01 ~ 2026-09-01 的模拟数据后重启服务；或使用"企业环境初始化器"生成 Demo 态企业。Demo 与 Live 双态隔离，不会污染真实数据。
...
| 局域网网关 | `start-hub.cmd`（0.0.0.0:8000） |
